...
# Logging parameters
RUN_NAME = "GPT_XTTS_v2.0_BBANGHYONG_FT_DNI"
...
# 텐서보드 로그가 저장될 경로
TENSORBOARD_LOG_DIR = os.path.join(OUT_PATH, "tensorboard_logs")
...
workspac = Path("/dev/hdd/user/kjy/ict/dataset_dni/datasets")
metadata = Path("/dev/hdd/user/kjy/ict/dataset_dni/datasets/text/대본1_modified.txt")
...
SPEAKER_REFERENCE = [
    "/dev/hdd/user/kjy/ict/dataset_dni/datasets/wavs/25.wav"  # speaker reference to be used in training test sentences
]
...
def main():
    ...
    config = GPTTrainerConfig(
        ...
        logger_uri=TENSORBOARD_LOG_DIR, # 텐서보드 로그 경로 설정
        audio=audio_config,
